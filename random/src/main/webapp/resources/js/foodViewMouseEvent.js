function foodViewMouseListener(){
	...
	this.centerPosition1;
	var circles = []; // 클릭으로 그려진 원과 반경 정보를 표시하는 선과 커스텀오버레이를 가지고 있을 배열입니다

	// 지도에 클릭 이벤트를 등록합니다
	 position = daum.maps.event.addListener(map, 'click', function(mouseEvent) {
	        removeCircles();
	    ...
	    if (!drawingFlag) {    
	       ...
	       centerPosition1 = centerPosition;
	...
	daum.maps.event.addListener(map, 'rightclick', function (mouseEvent) {
	    if (drawingFlag) {
	        ...
	        distance = Math.round(circle.getRadius());
